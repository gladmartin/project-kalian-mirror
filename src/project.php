...
// old version
$parsedown = new Parsedown();
$readmeHtml = $parsedown->text($readmeMd);
$crawler = new Crawler($readmeHtml);
$projects = [];
$crawler->filter('body > h3')->each(function ($node) use (&$projects) {
    $periode = trim($node->text());
    $items = [];
    $itemHtml = '<body>' . $node->nextAll()->html() . '</body>';
    $itemCrawl = new Crawler($itemHtml);
    $itemCrawl->filter('body > li')->each(function ($node) use (&$items, &$periode) {
        $author = $node->filter('p > strong');
        $author = $author->count() > 0 ? $author->text() : 'No Name';
        $author = trim($author);
        $initial = 'NN';
        if ($author != null) {
            $initial = substr($author, 0, 1) . substr($author, -1);
        ...
        $descriptionFull = $node->html();
        $linkElement = $node->filter('a');
        $link = $linkElement->attr('href');
        $linkElement = $linkElement->eq(0);
        foreach ($linkElement->eq(0) as $value) {
            $value->parentNode->removeChild($value);
        ...
        $githubLink = null;
        $node->filter('p')->each(function ($node) use (&$githubLink) {
            $text = $node->text();
            if (strpos($text, 'GitHub : [') !== false) {
                $githubLink = $node->filter('a')->attr('href');
            }
